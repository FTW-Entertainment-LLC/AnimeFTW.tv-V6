...
class Applications extends Config {
	...
	{
		$query = "SELECT `applications_submissions`.`id`, `applications_submissions`.`positionID`, `applications_submissions`.`company`, `applications_submissions`.`Age`, `applications_submissions`.`Status`, `users`.`ID` FROM `" . $this->MainDB . "`.`applications_submissions`, `" . $this->MainDB . "`.`users` WHERE `users`.`Username`= `applications_submissions`.`username` AND `applications_submissions`.`appRound` = " . $this->AppSettings[8]['value'];
		$result = mysql_query($query);
		echo $query;
		...
		{	if($i % 2)
			...
			<div class="table-row"' . $style . '>
				<div>
					...
					<div style="min-width:170px;display:inline-block;">' . $this->displaySecurityTest($row['ID']) . '</div>
	...
	private function displaySecurityTest($uid)
	{
		...
		if($count > 0)
		{
			...
			return '<span title="Test Uploaded on ' . date('m j, Y g:i',$row['date']) . '">Security Test</span>';
		}
		else
		{
			return '<span stsyle="color:gray;">Security Test</span>';
		}
	}
}
